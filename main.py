...
class UIObject():
    ...
    def draw(self, surface):
        ...
        """
        # Create surface for subobjects
        # Draw subobjects and their subobjects
        # Blit surface to given surface
        pass



class Placeholder(UIObject):
    ...
    def draw(self, surface):
        ...
        self.surface.fill((0, 0, 0, 0), self.placement)
...
class Button(UIObject):
    def __init__(self, placement:tuple, parent_position:tuple, color, alpha, text, font="monospace", font_size=10, font_color=(0, 0, 0), click_function=None):
        super().__init__(placement, color, alpha)
        self.font = font
    ...
    def clicked(self, pos):
        if pos[0] > self.x + self.parent_position[0] and pos[0] < self.x + self.width + self.parent_position[0]:
            if pos[1] > self.y + self.parent_position[1]  and pos[1] < self.y + self.height + self.parent_position[1] :
...
class Checkbox(UIObject):
    def __init__(self, placement:tuple, parent_position:tuple, checkbox_color, indicator_color, alpha, text, spacing=10, font="monospace", font_size=10, font_color=(0, 0, 0), click_function=None):
        super().__init__(placement, (0, 0, 0), alpha)
        self.font = font
        self.font_size = font_size
        self.font_color = font_color
        self.text = text
...
class Slider(UIObject):
    def __init__(self, placement:tuple, parent_position:tuple, min_value, max_value, jump, default_value, slider_color, bar_color, slider_radius, alpha, text, spacing=10, font="monospace", font_size=10, font_color=(0, 0, 0), click_function=None):
        super().__init__(placement, (0, 0, 0), alpha)
        self.font = font
        self.font_size = font_size
        self.font_color = font_color
    ...
    def clicked(self, pos):
        if pos[0] + self.slider_radius > self.x + self.parent_position[0] and pos[0] - self.slider_radius < self.x + self.width + self.parent_position[0]:
            if pos[1] + self.slider_radius > self.y + self.parent_position[1]  and pos[1] - self.slider_radius  < self.y + self.height + self.parent_position[1] :
                # Set new value
                bar_x_pos = self.position[0] + self.parent_position[0]
                distance = pos[0] - bar_x_pos
                pixels_per_one_jump = (self.placement[2]/((self.max_value - self.min_value)/self.jump))
                new_value = round(distance/pixels_per_one_jump, 0)*self.jump + self.min_value
